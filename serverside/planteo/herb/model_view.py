...
class PlantSuggestionAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        current_month = Month.objects.get(name=current_month_name)
        ...
        try:
            ...
            top_indices = np.argsort(probs[0])[-30:][::-1]
            top_10_plants = class_labels[top_indices]
            print(top_10_plants)
            plant_details = []
            for plant in top_10_plants:
                try:
                    herb = Herb.objects.get(common_name=plant.capitalize(), season_in_pakistan=current_month)
                    plant_details.append({
                        'id': herb.id,
                        'common_name': herb.common_name,
                        'description': herb.description,
                        'image': f'/media/{herb.image}',
                        'icons': f'/media/{herb.icons}',
                    })
